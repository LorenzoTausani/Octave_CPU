    ...
    if compute_new==1:
      ...
      for row in range(n_digits):
        for col in range(row,n_digits):
          d, d_cl, df_average,df_sem, Transition_matrix_rowNorm = Ian.generate_chimera_lbl_biasing(VGG_cl,elements_of_interest = [row,col], nr_of_examples = 1000, temperature = 1, plot=0, entropy_correction= entropy_correction)
